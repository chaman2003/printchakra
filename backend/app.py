...
# ============================================================================
# ORCHESTRATION ENDPOINTS (Print & Scan Workflows)
# ============================================================================

@app.route('/orchestrate/scan', methods=['POST'])
def orchestrate_scan():
    """
    Orchestrate scan workflow: print blank page → wait for scan → capture document
    This endpoint integrates printer and camera capture for document scanning
    """
    try:
        data = request.get_json() if request.is_json else {}
        
        # Extract scan options
        page_mode = data.get('pageMode', 'single')
        custom_range = data.get('customRange', '')
        ...
        print(f"\n{'='*60}")
        print(f"📸 SCAN ORCHESTRATION STARTED")
        print(f"{'='*60}")
        ...
        # Step 1: Initialize printer and verify blank.pdf exists
        blank_pdf = os.path.join(PRINT_DIR, 'blank.pdf')
        
        if not os.path.exists(blank_pdf):
            return jsonify({
                'status': 'error',
                'message': 'blank.pdf not found in print_scripts folder',
                'action': 'Please ensure blank.pdf exists before scanning'
            }), 404
        
        print(f"✓ Blank PDF verified: {blank_pdf}")
        
        # Step 2: Execute print script to print blank page
        print_script = os.path.join(PRINT_DIR, 'print-file.py')
        
        if not os.path.exists(print_script):
            return jsonify({
                'status': 'error',
                'message': 'print-file.py not found',
                'path': print_script
            }), 404
        
        print(f"✓ Print script found: {print_script}")
        
        try:
            # Execute print command
            print("🖨️  Sending blank page to printer...")
            result = subprocess.run(
                [sys.executable, print_script],
                cwd=PRINT_DIR,
                capture_output=True,
                text=True,
                timeout=30
            ...
            print(f"Print script output:\n{result.stdout}")
            if result.stderr:
                print(f"Print script errors:\n{result.stderr}")
            
            if result.returncode == 0:
                print("✓ Blank page sent to printer successfully")
            else:
                print(f"⚠ Print script returned code {result.returncode}")
                
        except subprocess.TimeoutExpired:
            print("⚠ Print command timed out (30s)")
            return jsonify({
                'status': 'error',
        ...
        # Step 3: Send Socket.IO notification to phone/camera to capture
        try:
            socketio.emit('scan_ready', {
                'message': 'Blank page printed. Please scan your document now.',
                'mode': page_mode,
                'settings': {
                    'layout': layout,
            ...
            print("✓ Scan notification sent to connected clients")
        except Exception as socket_error:
            print(f"⚠ Socket.IO notification failed (non-critical): {str(socket_error)}")
        
        # Step 4: Return success response
        return jsonify({
            'status': 'success',
            'message': 'Scan workflow initiated successfully',
            'details': {
                'blank_page_printed': True,
                'scan_notification_sent': True,
                'mode': page_mode,
                'settings': {
                    'layout': layout,
                    'paperSize': paper_size,
            ...
            'next_steps': 'Place your document on the blank page and capture using the phone camera'
        })
        
    except Exception as e:
        print(f"❌ Scan orchestration error: {str(e)}")
        traceback.print_exc()
        return jsonify({
            'status': 'error',
            'message': str(e)
...
def orchestrate_print():
    """
    Orchestrate print workflow: process files → convert to PDF → send to printer
    """
    try:
        # This endpoint would handle printing selected files
        # For now, return a placeholder response
        return jsonify({
            'status': 'success',
            'message': 'Print orchestration endpoint (not yet implemented)',
            'note': 'Use /print endpoint for basic printing'
        })
        
    except Exception as e:
        print(f"❌ Print orchestration error: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': str(e)
        }), 500
