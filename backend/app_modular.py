...
from modules import file_converter, scanning, storage
...
# Import services
from services import ConversionService, FileService, OCRService, PrintService, ScanService
...
def create_app(config_class=Config):
    ...
    file_service = FileService(
        upload_dir=app.config["UPLOAD_FOLDER"], processed_dir=app.config["PROCESSED_FOLDER"]
    )
    ...
    ocr_service = OCRService(processed_dir=app.config["PROCESSED_FOLDER"])

    conversion_service = ConversionService(
        converter_module=file_converter, converted_dir=app.config["CONVERTED_FOLDER"]
    )
